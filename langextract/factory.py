...
def create_model(
    ...
    response_model: type["PydanticBaseModel"] | None = None,
    ...
    Args:
      ...
      examples: Optional examples for schema generation.
      response_model: Optional Pydantic model for schema generation.
        Takes precedence over examples when use_schema_constraints=True.
...
def _create_model_with_schema(
    ...
    schema constraints derived from the provided examples or a Pydantic model.
    It also computes appropriate fence defaulting based on the schema's
    capabilities.

    Schema precedence:
      1. response_model (Pydantic model) - highest priority
      2. examples (auto-generate from ExampleData)
      3. No schema
    ...
    if use_schema_constraints:
        ...
        if schema_class is not None:
            # Precedence: response_model > examples
            if response_model is not None:
                try:
                    schema_instance = schema_class.from_pydantic(response_model)
                except NotImplementedError:
                    warnings.warn(
                        f"Provider {provider_class.__name__} does not support "
                        "Pydantic schemas. Falling back to examples-based schema.",
                        UserWarning,
                        stacklevel=3,
                    )
                    if examples:
