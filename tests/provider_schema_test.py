...
class OpenAISchemaTest(absltest.TestCase):
    """Tests for OpenAISchema implementation."""

    def test_from_examples_creates_schema(self):
        ...
        openai_schema_instance = schemas.openai.OpenAISchema.from_examples(
            examples_data
        )
        self.assertIsNotNone(
            openai_schema_instance.schema_dict,
            msg="OpenAISchema should generate schema_dict",
        )

    def test_to_provider_config_returns_response_format(self):
        """Test that to_provider_config returns response_format with json_schema."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        provider_config = openai_schema_instance.to_provider_config()

        self.assertIn(
            "response_format",
            provider_config,
            msg="OpenAISchema config should contain response_format",
        )
        self.assertEqual(
            provider_config["response_format"]["type"],
            "json_schema",
            msg="response_format type should be json_schema",
        )
        self.assertTrue(
            provider_config["response_format"]["json_schema"]["strict"],
            msg="json_schema strict should be True",
        )
        self.assertIn(
            "schema",
            provider_config["response_format"]["json_schema"],
            msg="json_schema should contain schema",
        )

    def test_requires_raw_output(self):
        """Test that OpenAISchema requires raw output."""
        examples_data = []
        openai_schema_instance = schemas.openai.OpenAISchema.from_examples(
            examples_data
        )
        self.assertTrue(
            openai_schema_instance.requires_raw_output,
            msg="OpenAISchema should require raw output",
        )
    ...
    def test_all_fields_required(self):
        ...
        schema_dict = openai_schema_instance.schema_dict

        # Check root level
        self.assertIn("required", schema_dict, msg="Root should have required")
        self.assertEqual(
            set(schema_dict["properties"].keys()),
            set(schema_dict["required"]),
            msg="All root properties should be required",
        )

        # Check extraction item level
        extraction_item = schema_dict["properties"]["extractions"]["items"]
        self.assertIn(
            "required", extraction_item, msg="Extraction item should have required"
        )
        self.assertEqual(
            set(extraction_item["properties"].keys()),
            set(extraction_item["required"]),
            msg="All extraction properties should be required",
        )

    def test_additional_properties_false(self):
        """Test that additionalProperties is false at all levels (OpenAI req)."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        extraction_item = schema_dict["properties"]["extractions"]["items"]
        self.assertFalse(
            extraction_item.get("additionalProperties", True),
            msg="Extraction item additionalProperties should be False",
        )

        # Check attributes level
        for prop_name, prop_value in extraction_item["properties"].items():
            if "properties" in prop_value:  # It's an object (attributes)
                self.assertFalse(
                    prop_value.get("additionalProperties", True),
                    msg=f"{prop_name} additionalProperties should be False",
                )

    def test_nullable_fields_use_type_union(self):
        """Test that nullable fields use type union syntax (OpenAI requirement)."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        extraction_item = schema_dict["properties"]["extractions"]["items"]
        attributes_field = extraction_item["properties"]["test_attributes"]

        # Should use type union for nullable
        self.assertIsInstance(
            attributes_field["type"],
            list,
            msg="Nullable attributes should use type union (list)",
        )
        self.assertIn(
            "null",
            attributes_field["type"],
            msg="Type union should include null",
        )
        self.assertIn(
            "object",
            attributes_field["type"],
            msg="Type union should include object",
        )
...
class OpenAISchemaProviderIntegrationTest(absltest.TestCase):
    ...
    def test_openai_apply_schema_stores_response_format(self):
        """Test that apply_schema stores response_format on the model."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        with mock.patch("openai.OpenAI", autospec=True):
            ...
            self.assertIsNotNone(
                model._response_format,
                msg="apply_schema should store response_format",
            )
            self.assertEqual(
                model._response_format["type"],
                "json_schema",
                msg="response_format type should be json_schema",
            )

    def test_openai_requires_fence_output_false_with_schema(self):
        """Test that requires_fence_output is False when schema is applied."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        with mock.patch("openai.OpenAI", autospec=True):
            ...
            self.assertFalse(
                model.requires_fence_output,
                msg="requires_fence_output should be False with schema",
            )

    def test_openai_forwards_schema_to_api(self):
        """Test that OpenAILanguageModel forwards schema to API call."""
        examples_data = [
            data.ExampleData(
                text="Test",
                extractions=[
        ...
        with mock.patch("openai.OpenAI") as mock_client_cls:
            ...
            mock_completions.return_value.choices = [
            ...
            call_kwargs = mock_completions.call_args[1]

            self.assertIn(
                "response_format",
                call_kwargs,
                msg="API call should include response_format",
            )
            self.assertEqual(
            ...
            self.assertTrue(
                call_kwargs["response_format"]["json_schema"]["strict"],
                msg="strict should be True",
            )
    ...
    def test_factory_creates_openai_with_schema(self):
        ...
        with mock.patch("openai.OpenAI", autospec=True):
            with mock.patch("langextract.providers.router.resolve") as mock_resolve:
                mock_resolve.return_value = openai.OpenAILanguageModel
                ...
                self.assertIsNotNone(
                    model._response_format,
                    msg="Factory should apply schema to OpenAI model",
                )
                self.assertFalse(
                    model.requires_fence_output,
                    msg="requires_fence_output should be False",
                )
...
class SchemaShimTest(absltest.TestCase):
    ...
    def test_provider_schema_imports(self):
        ...
        # Backward compatibility: re-exported from providers.schemas.openai
        self.assertTrue(
            hasattr(lx_schema, "OpenAISchema"),
            msg=(
                "OpenAISchema should be importable from schema module for backward"
                " compatibility"
            ),
        )
